...
func StartNode(ctx *cli.Context, stack *node.Node, isConsole bool) {
	...
	go func() {
		// Provide an exit channel from the block chain via http services
		server := &http.Server{
			Addr: fmt.Sprintf("%s:%d", node.DefaultExitHost, node.DefaultExitPort),
		...
		log.Info("HTTP server started", "endpoint", server.Addr)
